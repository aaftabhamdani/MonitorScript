#!/bin/bash
cat host.txt  | cut -d " " -f2 >hosts.txt
cat > index.html <<EOF
<!DOCTYPE html>
<html>
<head>
...
<div class="dropdown">
<button class="btn btn-primary dropdown-toggle" type="button" data-toggle="dropdown">List of Servers
<span class="caret"></span></button>
<ul class="dropdown-menu">
EOF
for dropdown in `cat hosts.txt`
do
echo '<li><a href='"./$dropdown/index1.html"'>'$dropdown'</a></li>' >> index.html
done
echo '</ul>' >> index.html
echo '</div>' >> index.html 
echo '</div>' >> index.html
...
scp -i  test.pem host.txt  basicinfo.sh anstohtml.sh  $user@"$ip":~ &>/dev/null
done < host.txt

while read j
do
user1=$(echo $j | cut -d " " -f1)
ip1=$(echo $j | cut -d " " -f2)
ssh -i test.pem $user1@"$ip1" < test.sh
ssh -i test.pem $user1@"$ip1" < ou.sh
ssh -i test.pem $user1@"$ip1" < og.sh
ssh -i test.pem $user1@"$ip1" < dash.sh
done < host.txt
echo "Remember first open index.html always"


while read k
do
user3=$(echo $k | cut -d " " -f1)
ip3=$(echo $k | cut -d " " -f2)

mkdir $ip3
cp -f bg.png $ip3/
cp -f  $ip3/bg.png ..
scp -i test.pem $user3@"$ip3":~/*.html $ip3/
done < host.txt
clear
echo "Please Open index.html file in your browser"
...
echo '</pre>' >> index.html
echo '<script type="text/javascript" charset="utf8" src="http://ajax.aspnetcdn.com/ajax/jQuery/jquery-1.8.2.min.js"></script>' >> index.html
echo '<script type="text/javascript" charset="utf8" src="http://ajax.aspnetcdn.com/ajax/jquery.dataTables/1.9.4/jquery.dataTables.min.js"></script>' >> index.html
echo '<script>' >> index.html
echo '$(function(){' >> index.html
echo '$("#example1").dataTable();' >> index.html
echo '})' >> index.html
echo '</script>' >> index.html
rm -f hosts.txt
